...
#### Github link
<https://github.com/andreasalvati/Bank-Campaign-Classification>
...
#### NA Discovery
We need to clean NA values in all the columns and impute the empty cells with proper methods based on each variable's condition.
First, we explore which columns have missing values. However, we noticed that our dataset is clean and no missing values are present and we could proceed directly to the EDA and feature engineering.
...
#### EDA
For the EDA, we analyzed first the target variable to see the proportions of each class (yes/no) then we analyzed each variable separately along with the target variable to see how the target varies with changes in independent variables.
```{r Target Variable}
...
# Based on the dataset describtioj, for pdays, 999 means that the client was not contacted before so we will replace 999 with zeroes
...
Along with the binning that was previously done in the visualization process, we will convert the target variable intro a binary variable of 0s and 1s and then factorizing it.
```{r}
...
We will now remove some variables that were previously binned and dummify the dataset for modeling purposes.
...
##### Evaluation Function 
We will construct an evaluation function that simply takes the name of the model and evaluates each based on the metrics underneath then appends it to a dataframe which allows for model comparison.
```{r}
...
##### Train control
Before modeling we will set our training control parameters as displayed below.
```{r train control}
...
##### XG BOOST
Let's build a XG Boost model using the grid parameters below and a train control of 5 cv. Note that the hyperparameters below have been iterated over many times before reaching the parameters for final search.
```{r XGBOOST}
xgbgrid <- expand.grid(
   nrounds=c(100, 200),
   max_depth = c(6, 10),
...
For Radial SVM we commented it out after evaluation due to long training time and noticing that it 
...
predictions_knn_smot <- ifelse(predictions_knn_smot == 1,0,1)
...
rf_smot <- caret::train(as.factor(y)~., data = training_rf,
                   ...
                   preProc = c("center", "scale"),
                   tuneGrid = tgrid, importance = 'impurity')
...
#### Displaying Models
The follwing shiny app allows the user to pick models and allows for comparison between them with respect to the calculated metrics!
```{r, echo=FALSE}
  ...
  server = function(input, output) {
    ...
    output$confusion_matrix <- renderPrint({
      ...
      cf_matrix<-confusionMatrix(data =
                                ...
                                reference=as.factor(validation_smoted$y))
    ...
    output$rocCurve <- renderPlot({
      ...
      plot(roc(predictor = all_models_prob[[input$model]][,1], response = validation_smoted$y), main = paste("AUC ",input$model))
...
After comparing models and defining the business case, we picked the best model (XGBoost) to predict our test dataset and export the results in a csv.
```{r Final Model}
cv_xgboost_smot$bestTune
